...
test_that("summarize_by_gene_symbol", {
  ...
  feature_gene_map <- data.table(featureid = features,
                                 genesymbol = genes)

  # fake gene expression data with features in a random order
  ge_norm <- data.table(feature_id = sample(features),
                      BS1 = runif(8),
                      BS2 = runif(8),
                      BS3 = runif(8))
  ge_summarized <- summarize_by_gene_symbol(ge_norm,
                                            feature_gene_map)
  ...
  expect_equal(ge_summarized[gene_symbol == "gene2"]$BS1,
               ge_norm[feature_id == "c"]$BS1)
  expect_equal(mean(ge_norm[feature_id %in% c("b", "d", "g")]$BS2),
